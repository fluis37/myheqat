CLASS zcl_03_eml DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .

  PUBLIC SECTION.
    INTERFACES if_oo_adt_classrun .

    CONSTANTS c_agency_id TYPE /dmo/agency_id VALUE '070003'.
    CONSTANTS c_travel_id TYPE /dmo/travel_id VALUE '00004203'.

  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.



CLASS zcl_03_eml IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    READ ENTITIES OF Z03_R_Travel
     ENTITY Travel
      ALL FIELDS WITH VALUE #( ( AgencyId = c_agency_id
                                 TravelId = c_travel_id ) )

      RESULT DATA(Travels_result)
      FAILED DATA(failed).

    IF failed IS NOT INITIAL.
      out->write( |Error lors de l'access Travel. | ).
    ELSE.
      out->write( Travels_result ).

      MODIFY ENTITIES OF Z03_R_Travel
       ENTITY Travel
        UPDATE FIELDS ( description )
        WITH VALUE #( ( AgencyId = c_agency_id
                        TravelId = c_travel_id
                        Description = 'new Description2' ) )
        REPORTED DATA(Travels_reported)
        FAILED failed.

      IF failed IS NOT INITIAL.
        out->write( |Error lors de la mise à jour Travel. | ).
        ROLLBACK ENTITIES.
      ELSE.
        COMMIT ENTITIES.
        out->write( travels_reported ).
        out->write( Travels_result ).
      ENDIF.

    ENDIF.
  ENDMETHOD.
ENDCLASS.
