...
CLASS lhc_item IMPLEMENTATION.
  ...
  METHOD ZZvalidateClass.
    ...
    READ ENTITIES OF z04_r_travel IN LOCAL MODE
      ENTITY item
      FIELDS ( AgencyId TravelId ZZClassZ04 )
    ...
    LOOP AT items ASSIGNING FIELD-SYMBOL(<item>).
      ...
      ELSE.
        SELECT SINGLE
          ...
          FIELDS classid
          WHERE classid = @<item>-ZZClassZ04 INTO @DATA(dummy).
        IF sy-subrc <> 0.
          APPEND VALUE #( %tky = <item>-%tky ) TO failed-item.
          APPEND VALUE #( %tky = <item>-%tky
                          %msg = NEW /lrn/cm_s4d437( textid   = /lrn/cm_s4d437=>class_not_exist
                                                     classid  = <item>-ZZClassZ04 )
                          %element-ZZClassZ04 = if_abap_behv=>mk-on
                          %state_area = c_area
                          %path-travel = CORRESPONDING #( <item> ) )
            TO reported-item.
...
CLASS lsc_Z04_R_TRAVEL IMPLEMENTATION.
  ...
  METHOD save_modified.
    ...
    LOOP AT update-item ASSIGNING FIELD-SYMBOL(<item>) WHERE %control-ZZClassZ04 = if_abap_behv=>mk-on.
      UPDATE z04_tritem
        SET zzclassz04 = @<item>-ZZClassZ04
        WHERE item_uuid = @<item>-ItemUuid.
    ENDLOOP.

    LOOP AT create-item ASSIGNING <item> WHERE %control-ZZClassZ04 = if_abap_behv=>mk-on.
      UPDATE z04_tritem
        SET zzclassz04 = @<item>-ZZClassZ04
        WHERE item_uuid = @<item>-ItemUuid.
    ENDLOOP.
